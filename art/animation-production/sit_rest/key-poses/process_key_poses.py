...
QA = ROOT / "qa"
NAMES = [
    ("k00_stand", "K00 source -> F00/F25 standing"),
    ("k04_descent", "K04 source -> F04 descent"),
    ("k06_seated_contact", "K06 v3 approved -> F08 seated"),
    ("k09_relaxed", "K09 source -> F12 relaxed (pending)"),
    ("k14_rise", "K14 source -> F22 rise (pending)"),
]
...
def standardize_sources() -> None:
    """Uniformly resize complete built-in outputs to the 1024px production canvas."""
    for stem, _ in NAMES[1:]:
        path = ROOT / f"{stem}_chroma.png"
...
def normalize_alpha() -> None:
    """Downsample the whole drawing, then align the complete sprite to the fixed pivot."""
    NORMALIZED.mkdir(parents=True, exist_ok=True)
    records: list[dict[str, object]] = []
    motion_scale = 0.89

    for stem, label in NAMES[1:]:
        source_path = ROOT / f"{stem}_alpha.png"
    ...
    report_path = QA / "normalization-report.json"
...
def build_contact_sheet() -> None:
    ...
    _save_png(sheet, QA / "sit_rest-key-contact-sheet.png")
