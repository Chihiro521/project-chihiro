...
QA = ROOT / "qa"
FRAMES = [
    (0, "F00 approved stand"),
    (4, "F04 deep descent v1"),
    (8, "F08 approved seated"),
    (22, "F22 controlled rise v1"),
    (25, "F25 exact F00 return"),
]
TARGET_VISIBLE_HEIGHTS = {4: 370, 22: 393}
TARGET_HEAD_CENTER_X = 256.0
TARGET_BASELINE_Y = 472

HELPER_PATH = KEY_POSES / "seated-loop-v3-root-locked" / "process_root_locked.py"
SPEC = importlib.util.spec_from_file_location("sit_rest_transition_helpers", HELPER_PATH)
...
def _source_for(index: int) -> Path:
    if index in (0, 25):
        return KEY_POSES / "normalized" / "k00_stand.png"
    if index == 8:
        return SIT_REST / "seated-loop-master-v1" / "frames" / "frame_008.png"
...
def normalize() -> None:
    ...
    for index, label in FRAMES:
        ...
        if index in (0, 25):
            ...
            bbox = helpers._binary_bbox(rgba)
            head_center_x = helpers._head_center_x(rgba, bbox)
            dx = round(TARGET_HEAD_CENTER_X - head_center_x)
            dy = TARGET_BASELINE_Y - (bbox[3] - 1)
            canvas = Image.new("RGBA", (512, 512), (0, 0, 0, 0))
            canvas.alpha_composite(rgba, (dx, dy))
            ...
            reduced = rgba.resize((scaled_side, scaled_side), Image.Resampling.LANCZOS)
            bbox = helpers._binary_bbox(reduced)
            head_center_x = helpers._head_center_x(reduced, bbox)
            dx = round(TARGET_HEAD_CENTER_X - head_center_x)
            dy = TARGET_BASELINE_Y - (bbox[3] - 1)
            canvas = Image.new("RGBA", (512, 512), (0, 0, 0, 0))
...
def contact_sheet() -> None:
    ...
    helpers._save_png(sheet, QA / "transition-key-contact-sheet.png")
